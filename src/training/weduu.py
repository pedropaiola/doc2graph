...
def e2e(args):
    ...
    #? test
    test_data = Document2Graph(name='WEDUU TEST', src_path=WEDUU_TEST, device = device, output_dir=TEST_SAMPLES)
    test_data.get_info()
    ...
    try:
        ...
        return {'LINKS [MAX, MEAN, STD]': [classes_f1[0], mean(edges_f1), np.std(edges_f1)], 'NODES [MAX, MEAN, STD]': [micro, mean(nodes_micro), np.std(nodes_micro)]}
    ...
    if not args.test:
        ...
        for cvs in cv_indices:
            ...
            for epoch in range(cfg_train.epochs):
                ...
                loss = compute_crossentropy_loss(scores.to(device), tg.edata['label'].to(device), device=device)
                auc = compute_auc_mc(scores.to(device), tg.edata['label'].to(device), device=device)
                ...
                with torch.no_grad():
                    ...
                    val_loss = compute_crossentropy_loss(val_scores.to(device), vg.edata['label'].to(device), device=device)
                    val_auc = compute_auc_mc(val_scores.to(device), vg.edata['label'].to(device), device=device)
